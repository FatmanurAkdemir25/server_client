public class DESAlgorithm {
    ...
    public String encrypt(String plaintext, String key) {
        ...
        long[] subKeys = generateSubKeys(bytesToLong(keyBytes));
        
        StringBuilder result = new StringBuilder();
        
        
        for (int i = 0; i < textBytes.length; i += 8) {
            ...
            long encrypted = encryptBlock(bytesToLong(block), subKeys);
    ...
    public String decrypt(String ciphertext, String key) {
        ...
        for (int i = 0; i < ciphertext.length(); i += 16) {
            String hexBlock = ciphertext.substring(i, Math.min(i + 16, ciphertext.length()));
            long encryptedBlock = Long.parseUnsignedLong(hexBlock, 16);
            
            long decrypted = decryptBlock(encryptedBlock, subKeys);
            byte[] decryptedBytes = longToBytes(decrypted);
    ...
    private long encryptBlock(long block, long[] subKeys) {
    ...
    private long decryptBlock(long block, long[] subKeys) {
    ...
    private int feistel(int right, long subKey) {
        ...
        int output = 0;
        for (int i = 0; i < 8; i++) {
            int input = (int) ((expanded >> (42 - i * 6)) & 0x3F);
        ...
        return (int) permute(output, P, 32);
    }
    ...
    private long permute(long input, int[] table, int inputSize) {
        return permute(input, table, inputSize, table.length);
    }
    ...
    private int permute(int input, int[] table, int inputSize) {
        int output = 0;
